...
* **⚡ Low-Latency Inference:** Generates complete, multi-channel marketing payloads in milliseconds using the Groq API.
...
* **AI Compute Layer:** Groq API (model configurable via `REACT_APP_GROQ_MODEL`, defaults to `openai/gpt-oss-120b` — originally built on `llama-3.3-70b-versatile`, migrated after Groq deprecated it on 8/16/26)
...
* **Client-side API key:** the Groq API is called directly from the browser (`dangerouslyAllowBrowser: true`), so `REACT_APP_GROQ_KEY` ships inside the client JS bundle and is visible to anyone who inspects the deployed site. This is fine for a demo, but a production version should proxy the call through a small backend/serverless function so the key never reaches the client.
* **No rate limiting:** every visitor shares the same Groq quota, so heavy traffic (or a scraped key) can exhaust it.
* **Personas are hardcoded:** the five personas in `src/data/personas.js` are illustrative sample data, not derived from real behavioral signals.
...
# Optional — override the model without touching code
